...
for(test.fold in 1:n.folds){
  is.test <- fold.vec == test.fold
  is.train <- !is.test
  x.train <- as.matrix(data.set$features[is.train,-5])
  x1<- x.train
  ...
  cv.list <- linearmodels:: LinearModelL1CV(as.numeric(x.train),y.train,sub.fold.vec,n.folds,penalty.vec,step.size)
  init.weight.vec <- cv.list$weight.vec
