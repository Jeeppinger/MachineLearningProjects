...
final.mat <- matrix(1:12, nrow = 5, dimnames = list(c("Set-1","Set-2","Set-3","Set-4","Set-5"), c("Early-Stop","L2-Predict","Baseline")))
#starts at 1
for(test.fold in 1:n.folds){
  is.test <- fold.vec == test.fold
    ...
    plot(sqr.loss.vec)
    sqr.loss.vec
    early.stop.list <- neuralnetwork:: NNetEarlyStoppingCV(x.train, y.train, sub.fold.vec, 30, step.size, n.hidden.units, n.folds)
    ...
    final.mat[test.fold,1] = early.stop.list$mean.validation.loss.vec
      #L2 regularized predictor
    final.mat[test.fold,2] = early.stop.list$mean.train.loss.vec
